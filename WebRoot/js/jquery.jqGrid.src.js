...
$.jgrid.extend({
	...
	insertNewRow : function(isCopyHh,defaultRow,locCol){
		var grid = $(this);
		var maxid = $.jgrid.randId();
		var idList = grid.jqGrid('getDataIDs');
		var rowLen = idList.length;
		if(rowLen>0 && isCopyHh){
			//取第一行的货品号、货品名称、单位
			if(rowLen == 0){
				return {};
			}
			
			var lastRowId = idList[rowLen-1];
			var lastRowData = grid.getRowData(lastRowId);
			defaultRow.goodNo = lastRowData.goodNo;
			defaultRow.goodName = lastRowData.goodName;
			defaultRow.unit = lastRowData.unit;
		}
		grid.jqGrid('addRowData', maxid, defaultRow );
		...
		if(nCol !== false) {
			var gridId = grid.attr('id'),nextRowNum = rowLen+1;
			setTimeout("jQuery('#"+gridId+"').editCell(" + nextRowNum + ", "+nCol+", true);", 100);
		}
	},
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$.jgrid.bindEv( elc, opt, $t);
				//对select来说，因默认选择第一条，故需触发change事件
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if(e.keyCode === 38){
						if ($t.p.iRow-1 >0 ) {
							...
							$($t).jqGrid("editCell",$t.p.iRow-1,$t.p.iCol,true);
							return false;
						}
					}
					if(e.keyCode === 40){
						if ($t.p.iRow+1 <=  $t.rows.length-1) {
							...
							if(!bufData.notAllow){
								...
								//默认可以新增
								//特殊条件下不能新增，如下裁
								$($t).jqGrid('insertNewRow', bufData.isCopyHp||false,bufData.defRow||{},bufData.sEditCol);
							}
						}
						return false;
					}
					...
					if(e.keyCode === 37){
						if ($t.p.iCol -1 >=  0) {
							...
							$($t).jqGrid("editCell",$t.p.iRow, i,true);
							return false;
						}
					}
					if(e.keyCode === 39 || e.keyCode === 13){
						if ($t.p.iCol +1 <=  $t.p.colModel.length-1) {
							...
							scrollGrid($t.p.iRow+1,i,'vd');
							$($t).jqGrid("editCell",$t.p.iRow+1,i,true);
						}
						return false;
					}
