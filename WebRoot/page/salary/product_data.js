 $(function () {
			...
			success:function(groupName){
				if(groupName.length > 0){ 
			        ...
			        for(var i=0; i<groupName.length; i++){
			        	firstPart.push({title:groupName[i], colspan:2});
			        	
			        	secondPart.push({field:'fetch'+groupName[i],title:'领用数',width:50,halign:'center',align:'right',editor:{type:'numberbox'}});
			        	secondPart.push({field:'comp'+groupName[i], title:'完工数',width:50,halign:'center',align:'right',editor:{type:'numberbox'}});
			        }
			        
			        firstPart.push({field:'sumSlFetch',    title:'领用数合计',hidden:false, halign:'center',align:'right', rowspan:2});
			        firstPart.push({field:'sumSlComp',     title:'完工数合计',hidden:false, halign:'center',align:'right', rowspan:2});
			        ...
			        //3.初始化Grid
					 ...
					 compHeader += '<th rowspan="2">领用数合计</th><th rowspan="2" >完工数合计</th></tr>';
	...
	function reloadProductData(){
		...
		$.ajax({
			async: false,
			url:'http://127.0.0.1:8080/cloth_erp/cgyb.action?repMonth='
		...
		$.ajax({
			async: false,
			url:'http://127.0.0.1:8080/cloth_erp/scjdyb.action?repMonth='
		...
		if(dataCgArray.length > 0 || dataQtArray.length > 0){
			...
			$.messager.progress('close');
			$.messager.show({msg:'本月没有数据!',title:'提示'});
			return;
		}
	}
		...
		$.ajax({
			...
			success:function(r){
				...
				if(r&&r == 'success'){
					...
					//回调计算
					$.ajax({
						url:'saveOrUpdateProductDataCallback',
						data:params,dataType:'json',type: "POST"});
